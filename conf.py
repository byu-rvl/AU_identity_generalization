...
parser.add_argument('--jobID', default=0, type=int, help="jobID")
parser.add_argument('--weights', default="None", type=str, help="weights for training")

# Network and Loss
parser.add_argument('--arc', default='swin_transformer_base', type=str, choices=['resnet18', 'resnet50', 'resnet101',
                    'swin_transformer_tiny', 'swin_transformer_small', 'swin_transformer_base', 'vit_b_16', 'vit_b_32', 
                    'vit_l_16', 'vit_l_32', 'inception_v3'], help="backbone architecture resnet / swin_transformer")
...
parser.add_argument('--lam_edge', default=228.0/0.37*0.01, type=float, help="lambda for adjusting loss")
parser.add_argument('--lam_contrasitive', default=228.0/1.34*0.01, type=float, help="lambda for adjusting loss")
parser.add_argument('--lam_lmk', default=228.0/221.0*0.01, type=float, help="lambda for adjusting loss")
...
def get_config():
    ...
    elif cfg.dataset == 'FEC':
        with open('config/FEC_config.yaml', 'r') as f:
            datasets_cfg = yaml.safe_load(f)
            datasets_cfg = edict(datasets_cfg)

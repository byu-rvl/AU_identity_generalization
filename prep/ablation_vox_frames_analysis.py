...
class find_static_faces:
    ...
    def update_worst_constraints(self, worst_lip_distance, worst_amnt_unrelaxed, worst_eye_open_amounts, worst_rolls, worst_pitches, worst_yaws, worst_max_pixel_highs, euler_angles, lip_distance, amnt_unrelaxed, eye_open_amount, num_pixels_high, frame_index, face_center, frame_name, face_index, total_faces):
        # Define thresholds for constraints
        max_roll = 15.0  # degrees
        max_pitch = 15.0  # degrees
        max_yaw = 15.0  # degrees
        ...
        if abs(roll) > max_roll and (len(worst_rolls) == 0 or abs(roll) > worst_rolls[0]):
            worst_rolls = [abs(roll), frame_index, face_center, frame_name, face_index, total_faces]
        if abs(pitch) > max_pitch and (len(worst_pitches) == 0 or abs(pitch) > worst_pitches[0]):
            worst_pitches = [abs(pitch), frame_index, face_center, frame_name, face_index, total_faces]
        if abs(yaw) > max_yaw and (len(worst_yaws) == 0 or abs(yaw) > worst_yaws[0]):
            worst_yaws = [abs(yaw), frame_index, face_center, frame_name, face_index, total_faces]
        if lip_distance > max_lip_distance and (len(worst_lip_distance) == 0 or lip_distance > worst_lip_distance[0]):
            worst_lip_distance = [lip_distance, frame_index, face_center, frame_name, face_index, total_faces]
        if amnt_unrelaxed > max_amnt_unrelaxed and (len(worst_amnt_unrelaxed) == 0 or amnt_unrelaxed > worst_amnt_unrelaxed[0]):
            worst_amnt_unrelaxed = [amnt_unrelaxed, frame_index, face_center, frame_name, face_index, total_faces]
        if eye_open_amount < min_eye_open_amount and (len(worst_eye_open_amounts) == 0 or eye_open_amount < worst_eye_open_amounts[0]):
            worst_eye_open_amounts = [eye_open_amount, frame_index, face_center, frame_name, face_index, total_faces]
        if num_pixels_high < min_num_pixels_high and (len(worst_max_pixel_highs) == 0 or num_pixels_high < worst_max_pixel_highs[0]):
            worst_max_pixel_highs = [num_pixels_high, frame_index, face_center, frame_name, face_index, total_faces]

        return worst_lip_distance, worst_amnt_unrelaxed, worst_eye_open_amounts, worst_rolls, worst_pitches, worst_yaws, worst_max_pixel_highs
    ...
    def find_best_frame_list(self, list_of_frames, frame_names = []):
        ...
        worst_lip_distance = []
        worst_amnt_unrelaxed = []
        worst_eye_open_amounts = []
        worst_rolls = []
        worst_pitches = []
        worst_yaws = []
        worst_max_pixel_highs = []
        ...
        for indx, (frame_name, frame) in enumerate(zip(frame_names, list_of_frames)):
            ...
            for i, (landmark, euler_angles) in enumerate(zip(landmarks, all_euler_angles)):
                ...
                if pass_constraints == 0:
                    ...
                    worst_lip_distance, worst_amnt_unrelaxed, worst_eye_open_amounts, worst_rolls, worst_pitches, worst_yaws, worst_max_pixel_highs = self.update_worst_constraints(worst_lip_distance, worst_amnt_unrelaxed, worst_eye_open_amounts, worst_rolls, worst_pitches, worst_yaws, worst_max_pixel_highs, euler_angles, lip_distance, amnt_unrelaxed, eye_open_amount, num_pixels_high, indx, self.get_face_center(landmark, frame_width, frame_height), frame_name, i, len(landmarks))
        ...
        percentile_best_frame_return = []
        percentile_best_frame_center = []
        done_indices = set()
        for i in range(100):
            perc_threshold = np.percentile(final_scores, i)
            perc_indices = np.where(final_scores <= perc_threshold)[0]
            if len(perc_indices) == 0:
                continue
            perc_index = perc_indices[0]
            # print(perc_threshold, perc_index, len(final_scores), len(perc_indices))
            perc_frame_name = list(results.keys())[perc_index]
            perc_metrics = results[perc_frame_name]
            perc_frame = list_of_frames[perc_metrics[0]]
            perc_frame_center = perc_metrics[5]
            if perc_index in done_indices:
                continue
            done_indices.add(perc_index)
            percentile_best_frame_return.append(perc_frame)
            percentile_best_frame_center.append(perc_frame_center)
        ...
        # worst_yaws = []
        # worst_max_pixel_highs = []
        if len(worst_lip_distance) > 0:
            worst_lip_frame = list_of_frames[worst_lip_distance[1]]
            worst_lip_framce_center = worst_lip_distance[2]
        else:
            worst_lip_frame = []
            worst_lip_framce_center = []
        if len(worst_amnt_unrelaxed) > 0:
            worst_amnt_unrelaxed_frame = list_of_frames[worst_amnt_unrelaxed[1]]
            worst_amnt_unrelaxed_frame_center = worst_amnt_unrelaxed[2]
        else:
            worst_amnt_unrelaxed_frame = []
            worst_amnt_unrelaxed_frame_center = []
        if len(worst_eye_open_amounts) > 0:
            worst_eye_open_frame = list_of_frames[worst_eye_open_amounts[1]]
            worst_eye_open_frame_center = worst_eye_open_amounts[2]
        else:
            worst_eye_open_frame = []
            worst_eye_open_frame_center = []
        if len(worst_rolls) > 0:
            worst_roll_frame = list_of_frames[worst_rolls[1]]
            worst_roll_frame_center = worst_rolls[2]
        else:
            worst_roll_frame = []
            worst_roll_frame_center = []
        if len(worst_pitches) > 0:
            worst_pitch_frame = list_of_frames[worst_pitches[1]]
            worst_pitch_frame_center = worst_pitches[2]
        else:
            worst_pitch_frame = []
            worst_pitch_frame_center = []
        if len(worst_yaws) > 0:
            worst_yaw_frame = list_of_frames[worst_yaws[1]]
            worst_yaw_frame_center = worst_yaws[2]
        else:
            worst_yaw_frame = []
            worst_yaw_frame_center = []
        if len(worst_max_pixel_highs) > 0:
            worst_pixel_high_frame = list_of_frames[worst_max_pixel_highs[1]]
            worst_pixel_high_frame_center = worst_max_pixel_highs[2]
        else:
            worst_pixel_high_frame = []
            worst_pixel_high_frame_center = []
        ...
        return percentile_best_frame_return, percentile_best_frame_center, worst_lip_frame, worst_lip_framce_center, worst_amnt_unrelaxed_frame, worst_amnt_unrelaxed_frame_center, worst_eye_open_frame, worst_eye_open_frame_center, worst_roll_frame, worst_roll_frame_center, worst_pitch_frame, worst_pitch_frame_center, worst_yaw_frame, worst_yaw_frame_center, worst_pixel_high_frame, worst_pixel_high_frame_center
...
if __name__ == '__main__':
    ...
    vox_celeb2_dir_best_frames = "/home/andreww9/groups/grp_ensembleAU2/nobackup/autodelete/VoxCeleb2_all_train_best_frames_ablation/"
    ...
    for option in all_options:
        ...
        percentile_best_frame_return, percentile_best_frame_center, worst_lip_frame, worst_lip_framce_center, worst_amnt_unrelaxed_frame, worst_amnt_unrelaxed_frame_center, worst_eye_open_frame, worst_eye_open_frame_center, worst_roll_frame, worst_roll_frame_center, worst_pitch_frame, worst_pitch_frame_center, worst_yaw_frame, worst_yaw_frame_center, worst_pixel_high_frame, worst_pixel_high_frame_center = analysis.find_best_frame_list(all_frames, all_frame_names)
        ...
        if len(percentile_best_frame_return) == 0:
        ...
        for i, (best_frame, best_frame_center) in enumerate(zip(percentile_best_frame_return, percentile_best_frame_center)):
            saveHere_percentile = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + f"_percentile_{i}.jpg")
            saveHere_percentile.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_percentile), best_frame)
            np.save(str(saveHere_percentile).replace(".jpg", "_face_center.npy"), np.array(best_frame_center))

        if len(worst_lip_frame) > 0:
            saveHere_worst_lip = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_lip.jpg")
            saveHere_worst_lip.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_lip), worst_lip_frame)
            np.save(str(saveHere_worst_lip).replace(".jpg", "_face_center.npy"), np.array(worst_lip_framce_center))
        if len(worst_amnt_unrelaxed_frame) > 0:
            saveHere_worst_amnt_unrelaxed = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_amnt_unrelaxed.jpg")
            saveHere_worst_amnt_unrelaxed.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_amnt_unrelaxed), worst_amnt_unrelaxed_frame)
            np.save(str(saveHere_worst_amnt_unrelaxed).replace(".jpg", "_face_center.npy"), np.array(worst_amnt_unrelaxed_frame_center))
        if len(worst_eye_open_frame) > 0:
            saveHere_worst_eye_open = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_eye_open.jpg")
            saveHere_worst_eye_open.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_eye_open), worst_eye_open_frame)
            np.save(str(saveHere_worst_eye_open).replace(".jpg", "_face_center.npy"), np.array(worst_eye_open_frame_center))
        if len(worst_roll_frame) > 0:
            saveHere_worst_roll = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_roll.jpg")
            saveHere_worst_roll.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_roll), worst_roll_frame)
            np.save(str(saveHere_worst_roll).replace(".jpg", "_face_center.npy"), np.array(worst_roll_frame_center))
        if len(worst_pitch_frame) > 0:
            saveHere_worst_pitch = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_pitch.jpg")
            saveHere_worst_pitch.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_pitch), worst_pitch_frame)
            np.save(str(saveHere_worst_pitch).replace(".jpg", "_face_center.npy"), np.array(worst_pitch_frame_center))
        if len(worst_yaw_frame) > 0:
            saveHere_worst_yaw = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_yaw.jpg")
            saveHere_worst_yaw.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_yaw), worst_yaw_frame)
            np.save(str(saveHere_worst_yaw).replace(".jpg", "_face_center.npy"), np.array(worst_yaw_frame_center))
        if len(worst_pixel_high_frame) > 0:
            saveHere_worst_pixel_high = Path(vox_celeb_to_process_dir_best_frames) / (id_to_process + "_" + Path(option).name + "_worst_pixel_high.jpg")
            saveHere_worst_pixel_high.parent.mkdir(parents=True, exist_ok=True)
            cv2.imwrite(str(saveHere_worst_pixel_high), worst_pixel_high_frame)
            np.save(str(saveHere_worst_pixel_high).replace(".jpg", "_face_center.npy"), np.array(worst_pixel_high_frame_center))

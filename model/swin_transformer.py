"""
Code originally from https://github.com/CVI-SZU/ME-GraphAU file: model/swin_transformer.py
...
if __name__=="__main__":
    ...
    # from torchvision.models import swin_transformer as st

    # model2 = st.SwinTransformer(embed_dim=96, depths=[ 2, 2, 18, 2 ], num_heads=[ 3, 6, 12, 24 ],
    #              window_size=[7,7], patch_size=[4,4])
    # print(model2)
    ...
    model3 = swin_b(weights=weights)
    
    model3.permute = Identity()
    model3.avgpool = Identity()
    model3.flatten = Identity()
    model3.head = Identity()

    output3 = model3(input)
    print(model3)
    print(output.shape, output3.shape)
    #reshape output3 to be the same size as ouput
    output3 = output3.reshape(output.shape)
    print(output.shape, output3.shape)
    print(torch.equal(output, output3))
    print(torch.equal(output.flatten(), output3.flatten()))